...
class TestCentralBankInferenceServices(TestCase):
    ...
    def setUp(self):
        ...
        self.test_date_earlier = date(2025, 12, 10)
    ...
    def test_get_fall_back_rate_success(self):
        ...
        """
        result = get_fall_back_rate(CentralBankChoices.FRB, date(2025, 12, 20))
        assert result == 3.64
    ...
    def test_get_central_bank_effective_rate_no_price_error(self):
        ...
        with pytest.raises(MarketPriceModel.DoesNotExist) as exc_info:
            get_central_bank_effective_rate(CentralBankChoices.FRB, self.test_date)
        assert str(exc_info.value) == "MarketPriceModel matching query does not exist."
    ...
    def test_get_fall_back_rate_no_prices_error(self):
        ...
        assert str(exc_info.value) == "No rates found before 2025-12-15 for FRB"
    ...
    def test_get_fall_back_rate_with_none_prices(self):
        ...
        result = get_fall_back_rate(CentralBankChoices.FRB, date(2026, 1, 15))
        assert result == 3.5
    ...
    def test_calculate_probability_changes(self):
        ...
        previous_matrix: CentralBankProbabilityMatrix = {
            ...
            "meeting_dates": [date(2026, 3, 20), date(2026, 5, 1)],
            "probability_matrix": [
                {
        ...
        assert result == {
            ...
            "probability_matrix": [
                ...
                {
                    "expected_rate_step": 25,
                    "probabilities": [0.0, -0.02],
                },
            ],
        }

    def test_calculate_probability_changes_with_some_different_meeting_dates(self):
        """
        GIVEN current and previous probability matrices with different meeting dates
        (e.g., after a meeting has occurred)
        WHEN calculating probability changes
        THEN probabilities are aligned by meeting date, not by index
        """
        previous_matrix: CentralBankProbabilityMatrix = {
            "central_bank": CentralBankChoices.FRB,
            "meeting_dates": [
            ...
            "probability_matrix": [
                {
                    "expected_rate_step": -25,
                    "probabilities": [0.10, 0.12, 0.15],
                },
                {
                    "expected_rate_step": 0,
                    "probabilities": [0.70, 0.68, 0.65],
                },
                {
                    "expected_rate_step": 25,
        ...
        current_matrix: CentralBankProbabilityMatrix = {
            ...
            "meeting_dates": [
                date(2026, 3, 20),
                date(2026, 5, 1),
                date(2026, 6, 12),  # new meeting
            ],
            "probability_matrix": [
                {
                    "expected_rate_step": -25,
                    "probabilities": [0.15, 0.18, 0.20],
                },
                {
                    "expected_rate_step": 0,
                    "probabilities": [0.65, 0.62, 0.60],
                },
                {
                    "expected_rate_step": 25,
                    "probabilities": [0.20, 0.20, 0.20],
        ...
        assert result == {
            "central_bank": CentralBankChoices.FRB,
            "meeting_dates": [date(2026, 3, 20), date(2026, 5, 1), date(2026, 6, 12)],
            "probability_matrix": [
                {
                    "expected_rate_step": -25,
                    "probabilities": [0.03, 0.03, 0.2],
                },
                {
                    "expected_rate_step": 0,
                    "probabilities": [-0.03, -0.03, 0.6],
                },
                {
                    "expected_rate_step": 25,
                    "probabilities": [0.0, 0.0, 0.2],
                },
            ],
        }

    def test_calculate_probability_changes_with_all_different_meeting_dates(self):
        """
        GIVEN current and previous probability matrices with all different meeting dates
        WHEN calculating probability changes
        THEN probabilities and meeting dates are those of the current matrix
        """
        current_matrix: CentralBankProbabilityMatrix = {
            "central_bank": CentralBankChoices.FRB,
            "meeting_dates": [
        ...
        assert result == {
            "central_bank": CentralBankChoices.FRB,
            "meeting_dates": [date(2026, 1, 31), date(2026, 3, 20), date(2026, 5, 1)],
            "probability_matrix": [
                {
                    "expected_rate_step": -25,
                    "probabilities": [0.10, 0.12, 0.15],
